...
export class DateControl extends React.Component<Props> {
    private input = <DateInputButton value="Date" onClick={(e: React.MouseEvent<HTMLElement>) => { }} />
    render() {
        const { selectedDate, onDateChanged } = this.props;
        return (
            <DatePicker selected={selectedDate} onChange={onDateChanged} locale="enGB" customInput={this.input} />
        )
    }
}