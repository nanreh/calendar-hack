...
const PlanPicker: React.FC<Props> = ({ availablePlans, selectedPlan, planChangeHandler }) => {

    const handleChange = (event: React.ChangeEvent<{ value: unknown }>) => {
        const newSelection = availablePlans.find(p => p.name === event.target.value as string);
        if (newSelection) {
    ...
    const planOptions = availablePlans.map(ap => (
        <option key={ap.name} value={ap.name}>({ap.type}) {ap.name}</option>
    ));
