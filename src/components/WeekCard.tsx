...
export const WeekCard: React.FC<Props> = ({ racePlan, desc, week, units, swap, selectedDow, hoveringDow, selectedWeek, hoveringWeek, }) => {
    return (
        <div>
            {week.days.map((d, index) => (
                <div>
                    <DayCell key={key(d.date)}
                        date={d.date}
                        dayDetails={d.event}
                        units={units}
                        swap={swap}
                        selected={(selectedDow === format(d.date, 'EEEE')) || (selectedWeek === week.weekNum)}
                        hovering={hoveringDow === format(d.date, 'EEEE') || (hoveringWeek === week.weekNum)} />
                </div>
            ))}
        </div>
    );
