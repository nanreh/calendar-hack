...
export class Distance {
    ...
    get meters(): number { return this._meters; }
...
export function toDistance(raceType: RaceType): Distance {
    switch (raceType) {
        case "5K": return new Distance(5000);
        case "10K": return new Distance(10000);
        case "Half Marathon": return new Distance(21097.5);
        case "Marathon": return new Distance(42195.0);
    }
}

export const distances: { [key: string]: RaceDistance } = {
    "Marathon": { name: 'Marathon', distance: 42195, worldRecord: toSeconds(2, 1, 39), defaultTime: toSeconds(2, 59, 59) }, //  2:01:39
    "Half Marathon": { name: 'Half Marathon', distance: 21097.5, worldRecord: toSeconds(0, 58, 1), defaultTime: toSeconds(1, 29, 59) }, // 58:01
    "5K": { name: '5K', distance: 5000, worldRecord: toSeconds(0, 12, 51), defaultTime: toSeconds(0, 19, 59) }, //12:51
    "10K": { name: '10K', distance: 10000, worldRecord: toSeconds(0, 26, 24), defaultTime: toSeconds(0, 39, 59) }, // 26:24
...
export const humanizeDuration = function (durationSecs: number, opts: renderOpts): string {
    ...
    if (hours > 0 || opts.leadingZeroes) {
        toRender.push(opts.padHours ? hours.toFixed().padStart(2, '0') : hours.toFixed())
    }
    if (hours > 0 || minutes > 0 || opts.leadingZeroes) {
        toRender.push(opts.padMinutes ? minutes.toFixed().padStart(2, '0') : minutes.toFixed())
    }
    if (hours > 0 || minutes > 0 || seconds > 0 || opts.leadingZeroes) {
        toRender.push(opts.padSeconds ? seconds.toFixed().padStart(2, '0') : seconds.toFixed())
    }
    return toRender.join(':');
}

export const humanizeDistance = function (distanceMeters: number, units: Units): string {
    const d = units === 'mi' ? metersToMi(distanceMeters).toFixed(1) : metersToKm(distanceMeters).toFixed(1);
